...
import {
  ...
  CheckCircle2, Loader2, GripVertical, Calendar, X, Info
} from 'lucide-react';

// ─── PANNEAU DE PARTAGE ───────────────────────────────────────────────────────
const SharePanel = ({ story, onClose, whatsappNumber }) => {
  ...
  const shareOptions = [
    {
      name: 'WhatsApp',
      color: '#25D366',
      bg: 'bg-[#25D366]/10 hover:bg-[#25D366] hover:text-white',
      text: 'text-[#25D366]',
      auto: false,
      hint: 'Copie le lien + ouvre WhatsApp. Collez-le dans votre statut manuellement.',
      action: () => {
        copyLink();
        window.open(`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`${title} — ${storyUrl}`)}`, '_blank');
      },
      icon: (
    ...
    },
    {
      name: 'Facebook',
      color: '#1877F2',
      bg: 'bg-[#1877F2]/10 hover:bg-[#1877F2] hover:text-white',
      text: 'text-[#1877F2]',
      auto: true,
      hint: 'Partage direct via le dialogue Facebook.',
      action: () => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(storyUrl)}&quote=${encodeURIComponent(title)}`, '_blank', 'width=600,height=400'),
      icon: (
    ...
    },
    {
      name: 'Telegram',
      color: '#0088CC',
      bg: 'bg-[#0088CC]/10 hover:bg-[#0088CC] hover:text-white',
      text: 'text-[#0088CC]',
      auto: true,
      hint: 'Partage direct via Telegram.',
      action: () => window.open(`https://t.me/share/url?url=${encodeURIComponent(storyUrl)}&text=${encodeURIComponent(title)}`, '_blank'),
      icon: (
        <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      name: 'TikTok',
      color: '#000000',
      bg: 'bg-black/10 hover:bg-black hover:text-white dark:bg-white/10 dark:hover:bg-white dark:hover:text-black',
      text: 'text-black dark:text-white',
      auto: false,
      hint: 'Copie le lien. Collez-le en bio ou description TikTok manuellement.',
    ...
    },
    {
      name: 'Instagram',
      color: '#E1306C',
      bg: 'bg-[#E1306C]/10 hover:bg-gradient-to-br hover:from-[#f09433] hover:via-[#e6683c] hover:via-[#dc2743] hover:via-[#cc2366] hover:to-[#bc1888] hover:text-white',
      text: 'text-[#E1306C]',
      auto: false,
      hint: 'Copie le lien. Collez-le dans votre bio ou description Instagram manuellement.',
      ...
      <div className="w-full max-w-md bg-white dark:bg-neutral-900 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex items-center justify-between p-6 border-b border-black/5 dark:border-white/10">
          <div className="flex items-center gap-3">
            {story.media_url && (
              <img src={story.media_url} alt="" className="w-12 h-12 rounded-2xl object-cover" />
            )}
            <div>
        ...
        <div className="p-6 space-y-3">
          {shareOptions.map(opt => (
            <div key={opt.name}>
              <button
                onClick={opt.action}
                className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all ${opt.bg} ${opt.text} group`}
              >
                ...
                <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full ${opt.auto ? 'bg-green-500/20 text-green-600' : 'bg-orange-500/20 text-orange-600'}`}>
...
const Stories = () => {
  ...
  // Compression image côté client (< 300 Ko)
    ...
    try {
      ...
      if (form.media_file) {
        ...
        const { error: upErr } = await supabase.storage
          .from('products')
          .upload(fileName, form.media_file, { upsert: false });
      ...
      setShowForm(false);
      setForm({ title_fr: '', title_en: '', redirect_link: '', expires_at: '', media_url: '', media_file: null, preview: null });
      setSaved(true);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">
            {language === 'fr' ? 'Publiez et partagez vos contenus visuels.' : 'Publish and share your visual content.'}
          </p>
        </div>
        <button
      ...
      {/* FORMULAIRE NOUVELLE STORY */}
      {showForm && (
        <div className="bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[3rem] p-8 border border-white/20 shadow-2xl space-y-6 animate-in slide-in-from-top-4 duration-300">
          <div className="flex items-center justify-between">
            <h3 className="font-black text-gray-900 dark:text-white uppercase tracking-tight text-lg">Nouvelle Story</h3>
            <button onClick={() => setShowForm(false)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 text-gray-500"><X size={18}/></button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              {form.preview ? (
                <div className="relative w-full h-full">
                  ...
                  <button onClick={() => setForm(f => ({ ...f, media_file: null, preview: null }))} className="absolute top-3 right-3 p-1.5 bg-black/60 rounded-full text-white hover:bg-black"><X size={14}/></button>
                </div>
              ) : (
                <div className="w-full h-full flex flex-col items-center justify-center gap-3 text-gray-400 cursor-pointer" onClick={() => fileRef.current?.click()}>
                  <Upload size={32}/>
                  <p className="text-[10px] font-bold uppercase text-center px-4">Glisser-déposer ou cliquer<br/><span className="text-dakora-green">Max 300 Ko — compression auto</span></p>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-black uppercase text-gray-400 ml-2 flex items-center gap-1"><Calendar size={10}/> Date d'expiration</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-black uppercase text-gray-400 ml-2">Ou URL d'image / vidéo</label>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-black/5 dark:border-white/5">
            <button onClick={() => setShowForm(false)} className="px-6 py-3 rounded-2xl font-black uppercase text-xs text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 transition-all">Annuler</button>
            <button onClick={handlePublish} disabled={uploading} className="flex items-center gap-2 px-8 py-3 bg-dakora-green text-white rounded-2xl font-black uppercase text-xs shadow-lg hover:bg-green-700 transition-all disabled:opacity-50">
              {uploading ? <Loader2 size={16} className="animate-spin"/> : <Plus size={16}/>} Publier
            </button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center p-20 bg-white/40 dark:bg-white/5 rounded-[3rem] border border-dashed border-gray-300 dark:border-white/10 text-center">
          ...
          <p className="text-gray-400 italic font-bold">Aucune story. Publiez votre premier contenu !</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {stories.map(story => {
            ...
            return (
              <div key={story.id} className={`group relative bg-white/40 dark:bg-white/5 rounded-[2.5rem] overflow-hidden border-2 shadow-xl transition-all ${story.is_active && !(story.expires_at && new Date(story.expires_at) < new Date()) ? 'border-dakora-green' : 'border-gray-200 dark:border-white/10 opacity-70'}`}>
                <div className="aspect-video relative overflow-hidden bg-gray-100 dark:bg-neutral-800">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-5 gap-1">
                    ...
                    {story.redirect_link && <p className="text-dakora-green text-[10px] font-bold">{story.redirect_link}</p>}
                  </div>
                  {/* Statut badge */}
                ...
                <div className="p-4 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold">
                    {story.expires_at && (
                      <span className="flex items-center gap-1"><Calendar size={10}/>{new Date(story.expires_at).toLocaleDateString()}</span>
                    )}
                  </div>
