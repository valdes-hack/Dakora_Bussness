...
const SharePanel = ({ story, onClose, whatsappNumber, onShare }) => {
  ...
  const downloadImage = async (url, filename) => {
    try {
      ...
      console.warn("Échec du téléchargement CORS direct, ouverture dans un nouvel onglet:", err);
      window.open(url, '_blank');
    }
  };

  const handleNativeShare = async () => {
    setSharingNatively(true);
    try {
      const shareData = {
        title: title,
        text: `${title} — ${storyUrl}`,
      ...
      if (story.media_url) {
        try {
          ...
          console.warn("CORS ou canShare empêche le partage de fichier natif:", e);
        }
      }

      await navigator.share(shareData);
      onShare?.('Partage Natif');
    } catch (err) {
      if (err.name !== 'AbortError') {
        console.error("Erreur de partage natif:", err);
      }
    } finally {
      setSharingNatively(false);
    }
  };

  const shareOptions = [
    {
      name: 'WhatsApp (Mon Statut)',
      bg: 'bg-[#25D366]/10 hover:bg-[#25D366] hover:text-white', text: 'text-[#25D366]',
      auto: false, hint: 'WhatsApp s\'ouvrira : choisissez "Mon Statut" en haut de la liste.',
      action: () => { 
        copyLink(); 
        window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(`${title} — ${storyUrl}`)}`, '_blank'); 
    ...
    {
      name: 'Facebook (Ma Story)',
      bg: 'bg-[#1877F2]/10 hover:bg-[#1877F2] hover:text-white', text: 'text-[#1877F2]',
      auto: true, hint: 'Sélectionnez "Votre Story" sur Facebook pour la publier.',
      action: () => { 
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(storyUrl)}&quote=${encodeURIComponent(title)}`, '_blank', 'width=600,height=400'); 
        onShare?.('Facebook Story'); 
      },
    ...
    {
      name: 'Instagram (Ma Story)',
      bg: 'bg-[#E1306C]/10 hover:bg-[#E1306C] hover:text-white', text: 'text-[#E1306C]',
      auto: false, hint: 'Télécharge l\'image + copie le lien. Publiez-la dans votre Story Instagram.',
      action: () => { 
        copyLink(); 
        if (story.media_url) downloadImage(story.media_url, `story-${story.id}.jpg`);
        window.open('https://www.instagram.com/', '_blank'); 
        onShare?.('Instagram Story'); 
      },
    ...
    },
    {
      ...
      auto: false, hint: 'Télécharge l\'image + copie le lien. Publiez-la dans votre Story TikTok.',
      action: () => { 
        copyLink(); 
        if (story.media_url) downloadImage(story.media_url, `story-${story.id}.jpg`);
        window.open('https://www.tiktok.com/', '_blank'); 
        onShare?.('TikTok Story'); 
      },
    ...
    {
      name: 'Telegram (Mon Canal)',
      bg: 'bg-[#0088CC]/10 hover:bg-[#0088CC] hover:text-white', text: 'text-[#0088CC]',
      auto: true, hint: 'Partage direct via Telegram.',
      action: () => { 
        window.open(`https://t.me/share/url?url=${encodeURIComponent(storyUrl)}&text=${encodeURIComponent(title)}`, '_blank'); 
        onShare?.('Telegram'); 
      },
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-3 md:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-full max-w-md bg-white dark:bg-neutral-900 rounded-[1.5rem] md:rounded-[2rem] lg:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
        ...
        {/* Header */}
        <div className="flex items-center justify-between p-4 md:p-5 lg:p-6 border-b border-black/5 dark:border-white/10">
          <div className="flex items-center gap-2 md:gap-3">
            {story.media_url && (
              <img src={story.media_url} alt="" className="w-10 h-10 md:w-12 md:h-12 rounded-xl object-cover border border-black/5" />
            )}
            <div className="min-w-0">
              <h3 className="font-black text-gray-900 dark:text-white text-xs md:text-sm uppercase tracking-tight truncate max-w-[150px] md:max-w-[200px]">{title}</h3>
              <p className="text-[9px] md:text-[10px] text-gray-400 font-bold truncate max-w-[120px] md:max-w-[180px]">{storyUrl}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 transition-all flex-shrink-0">
            <X size={16}/>
          </button>
        </div>
        ...
        <div className="px-4 md:px-5 lg:px-6 pt-4 pb-2">
          <button onClick={copyLink} className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl md:rounded-2xl border-2 text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${copied ? 'border-dakora-green bg-dakora-green/5 text-dakora-green' : 'border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-300 hover:border-dakora-green'}`}>
            {copied ? <CheckCircle2 size={15}/> : <LinkIcon size={15}/>}
            {copied ? 'Lien copié !' : 'Copier le lien de la Story'}
          </button>
        </div>

        {/* Bouton Partage Natif Mobile (si disponible) */}
        {nativeShareSupported && (
          <div className="px-4 md:px-5 lg:px-6 pt-1 pb-2">
            <button 
              onClick={handleNativeShare} 
              disabled={sharingNatively}
              className="w-full flex items-center justify-center gap-2 py-3 bg-dakora-green text-white rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-widest shadow-lg hover:bg-green-700 active:scale-95 transition-all disabled:opacity-50"
            >
              {sharingNatively ? <Loader2 className="animate-spin" size={14}/> : <Share2 size={14}/>}
              Partage Natif (Mobile)
            </button>
          </div>
        )}

        {/* Options de partage individuel */}
        <div className="p-4 md:p-5 lg:p-6 space-y-2 max-h-[50vh] overflow-y-auto custom-scrollbar">
          {shareOptions.map(opt => (
            <button key={opt.name} onClick={opt.action} className={`w-full flex items-center gap-3 md:gap-4 px-4 py-3 rounded-xl md:rounded-2xl transition-all ${opt.bg} ${opt.text}`}>
